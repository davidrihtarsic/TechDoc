...
je alternativna interpretacija te iste projekcijske situacije. Le da v tem primeru ne rotiramo samega predmeta, pač pa rotiramo projekcijsko ravnino.Torej zanima nas kam moramo premakniti projekcijsko ravnino $\Pi_{iso}$ z normalnim vektorjem $\mathbf{n}$, da dobimo enak rezultat kot v prejšnji situaciji? Ker je rotacija linearna transformacija, lahko prejšnji normalni vektor $\mathbf{n_y}$ projekcijske ravnine $\Pi_{XZ}$ rotiramo z inverzno matriko $\mathbf{R}^{-1}$:

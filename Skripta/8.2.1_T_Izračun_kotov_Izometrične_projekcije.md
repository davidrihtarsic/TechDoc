...
### Rotacija vektorjev in projekcija
...
Situacijo lahko razumemo tudi iz drugega zornega kota, na primer kam moramo premakniti trenutno projekcijko ravnino $\Pi_{XY}$ z normalnim vektorjem $\mathbf{e_y=[0,0,1]^T}$, da dobimo enak rezultat. Zanimajo nas koordinate novega normalnega vektorja $\mathbf{n_{POV}}$ te projekcijske ravnine $\Pi_{POV}$. Ker imamo izračunano rotacijsko matriko $\mathbf{R}$ je to enostavno inverzna rotacija:

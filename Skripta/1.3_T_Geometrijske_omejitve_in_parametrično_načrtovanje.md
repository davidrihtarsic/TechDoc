...
## Parametrično modeliranje: omejitve, reference in spremenljivke
...
1. V **Sketcherju** dodaj dimenzijo (npr. premer kroga) in ji določi smiselno ime, na primer `d_nominalni`. Ime lahko določiš že ob ustvarjanju omejitve ali pa ga naknadno spremeniš v seznamu **Constraints**.
2. Poimenovana dimenzijska omejitev postane dostopna tudi med lastnostmi skice. Ko skico izberemo zunaj urejanja, jo najdemo v zavihku **Data → Constraints → d_nominalni**. Njeno vrednost lahko zato spremenimo neposredno med podatki skice, ne da bi morali ponovno odpreti skico in v njej poiskati ustrezno mero.
3. Isto ime lahko uporabimo tudi v drugih omejitvah ali lastnostih kot izraz: `Sketch.Constraints.d_nominalni` (v isti skici lahko pogosto tudi krajše `Constraints.d_nominalni`). Poimenovanje omejitev tako izboljša preglednost modela in omogoča, da pomembne mere obravnavamo kot jasno določene parametre modela.
4. Po potrebi katerokoli mero spremeni v **Reference** (Toggle reference → siva), če želiš, da je samo **izpis/izračun** in ne vpliva na geometrijo.
